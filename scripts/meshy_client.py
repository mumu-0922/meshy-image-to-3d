...
RIGGING_ENDPOINT = f"{MESHY_BASE_URL}/openapi/v1/rigging"
...
def retrieve_rigging_task(api_key: str, task_id: str) -> Dict[str, Any]:
    return json_request("GET", f"{RIGGING_ENDPOINT}/{task_id}", api_key)
...
def redact_urls_for_archive(value: Any, key: str = "") -> Any:
    if should_redact_value(key, value):
        return "<downloaded-and-redacted>"
    if isinstance(value, dict):
        return {child_key: redact_urls_for_archive(child_value, child_key) for child_key, child_value in value.items()}
    if isinstance(value, list):
        return [redact_urls_for_archive(item, key) for item in value]
    return value


def redact_task_for_archive(task: Dict[str, Any]) -> Dict[str, Any]:
    return redact_urls_for_archive(task)


def load_task_id_from_json(path: Path) -> str:
    if not path.exists():
        raise MeshyError(f"Task JSON does not exist: {path}")
    data = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(data, dict):
        raise MeshyError(f"Task JSON must be an object: {path}")
    task_id = data.get("id") or data.get("result")
    if not task_id:
        raise MeshyError(f"Task JSON missing id/result: {path}")
    return str(task_id)
...
def expected_rigged_fbx_path(out_dir: Path, asset_name: str) -> Path:
    return out_dir / "model" / f"{asset_name}_rigged.fbx"
...
def create_rigging_task(
    api_key: str,
    input_task_id: Optional[str],
    model_url: Optional[str],
    height_meters: float,
) -> str:
    if not input_task_id and not model_url:
        raise MeshyError("rig-character requires --input-task-id/--task-json or --model-url")
    ...
    response = json_request("POST", RIGGING_ENDPOINT, api_key, payload)
    task_id = response.get("result")
    if not task_id:
        raise MeshyError(f"Meshy rigging response missing result task id: {response}")
    return str(task_id)


def poll_rigging_task(
    api_key: str,
    task_id: str,
    interval_seconds: int,
    max_wait_seconds: int,
    ...
    while True:
        task = request_with_rate_limit_retry(
            "rig-poll",
            lambda: retrieve_rigging_task(api_key, task_id),
            retries=rate_limit_retries,
            backoff_seconds=rate_limit_backoff_seconds,
            log_prefix=log_prefix,
        )
        ...
        if status != last_status or status in {"PENDING", "IN_PROGRESS"}:
            print(f"{log_prefix} rig_task={task_id} status={status} progress={progress}%", flush=True)
            last_status = status
        ...
        if status in {"FAILED", "CANCELED", "EXPIRED"}:
            raise MeshyError(f"Meshy rigging task ended with status={status}: {task.get('task_error')}")

        if time.time() >= deadline:
            raise MeshyError(f"Timed out waiting for Meshy rigging task {task_id} after {max_wait_seconds}s")

        time.sleep(interval_seconds)


def write_rigging_readme(
    out_dir: Path,
    asset_name: str,
    input_task_id: Optional[str],
    rigging_task_id: str,
    ...
    source_line = f"- Input image-to-3D task id: `{input_task_id}`" if input_task_id else "- Input model: external `model_url`"
    readme = f"""# {asset_name} Meshy Rigged Character

Generated by `tools/ai3d/meshy_client.py rig-character`.

## Source

{source_line}
- Meshy rigging task id: `{rigging_task_id}`
- Height meters: `{height_meters}`

## Output

- Rigged FBX: `model/{asset_name}_rigged.fbx`
- Rigging task archive: `source/rigging-task.json`

## Unity checklist

- Import FBX as Humanoid.
- Use this new character Avatar.
- Reuse the existing runner animation clip/controller from `HunyuanRunner.fbx`.
- Disable root motion for the projection runner.
- Validate scale, feet contact, material readability, and lane-change lean in Play mode.
"""
    (out_dir / "README.md").write_text(readme, encoding="utf-8")


def generate_rigged_character(
    api_key: str,
    out_dir: Path,
    asset_name: str,
    input_task_id: Optional[str],
    ...
    skip_existing: bool,
    download_glb: bool,
    download_basic_animations: bool,
    rate_limit_retries: int,
    rate_limit_backoff_seconds: int,
) -> Tuple[str, Path, Optional[str]]:
    asset_name = asset_name.strip()
    ...
    rigged_fbx_path = expected_rigged_fbx_path(out_dir, asset_name)
    if skip_existing and rigged_fbx_path.exists() and rigged_fbx_path.stat().st_size > 0:
        print(f"{log_prefix} skip existing rigged FBX -> {rigged_fbx_path}", flush=True)
        return "skipped", rigged_fbx_path, None
    ...
    source_dir.mkdir(parents=True, exist_ok=True)
    model_dir.mkdir(parents=True, exist_ok=True)
    if download_basic_animations:
        animation_dir.mkdir(parents=True, exist_ok=True)

    print(f"{log_prefix} creating rigging task for {asset_name}", flush=True)
    rigging_task_id = request_with_rate_limit_retry(
        "rig-create",
        lambda: create_rigging_task(
            api_key=api_key,
    ...
    print(f"{log_prefix} created rigging task {rigging_task_id}", flush=True)

    task = poll_rigging_task(
        api_key=api_key,
    ...
    if not isinstance(result, dict):
        raise MeshyError(f"Succeeded rigging task has invalid result: {task}")

    fbx_url = result.get("rigged_character_fbx_url")
    if not fbx_url:
        raise MeshyError(f"Succeeded rigging task has no rigged_character_fbx_url: {task}")
    ...
    if download_glb and result.get("rigged_character_glb_url"):
        ...
        print(f"{log_prefix} downloading rigged GLB -> {rigged_glb_path}", flush=True)
        download_url(str(result["rigged_character_glb_url"]), rigged_glb_path)

    if download_basic_animations:
        animations = result.get("basic_animations") or {}
        if isinstance(animations, dict):
            for animation_key, animation_url in animations.items():
                if animation_url:
                    extension = "fbx" if str(animation_key).endswith("_fbx_url") else "glb"
                    animation_name = str(animation_key).replace("_url", "")
                    animation_path = animation_dir / f"{animation_name}.{extension}"
                    print(f"{log_prefix} downloading basic animation {animation_name} -> {animation_path}", flush=True)
                    download_url(str(animation_url), animation_path)

    safe_write_json(source_dir / "rigging-task.json", redact_task_for_archive(task))
    write_rigging_readme(out_dir, asset_name, input_task_id, rigging_task_id, height_meters)

    print(f"{log_prefix} done", flush=True)
    print(f"{log_prefix} rigged_model: {rigged_fbx_path}", flush=True)
    return "succeeded", rigged_fbx_path, rigging_task_id


def run_rig_character(args: argparse.Namespace) -> int:
    api_key = require_api_key(args.api_key_file)
    input_task_id = args.input_task_id.strip() if args.input_task_id else None
    if args.task_json:
        input_task_id = load_task_id_from_json(Path(args.task_json))

    generate_rigged_character(
        api_key=api_key,
        out_dir=Path(args.out),
        asset_name=args.name,
        input_task_id=input_task_id,
        ...
        poll_interval=args.poll_interval,
        max_wait=args.max_wait,
        log_prefix="[meshy:rig]",
        skip_existing=args.skip_existing,
        download_glb=args.download_glb,
        download_basic_animations=args.download_basic_animations,
        rate_limit_retries=args.rate_limit_retries,
        rate_limit_backoff_seconds=args.rate_limit_backoff,
    )
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    rig_parser = subparsers.add_parser("rig-character", help="Rig a Meshy humanoid character and download rigged FBX")
    rig_input_group = rig_parser.add_mutually_exclusive_group(required=True)
    rig_input_group.add_argument("--task-json", help="Redacted or raw image-to-3D task JSON containing id/result")
    rig_input_group.add_argument("--input-task-id", help="Meshy image-to-3D task id to rig")
    rig_input_group.add_argument("--model-url", help="Public GLB URL or data URI to rig; Meshy rigging model_url supports GLB")
    rig_parser.add_argument("--name", required=True, help="Asset slug/name, e.g. female_runner_pink_ponytail_01")
    rig_parser.add_argument("--out", required=True, help="Output folder, e.g. Assets/QuizRush/Generated/AI3D/name/rigged")
    rig_parser.add_argument("--height-meters", type=float, default=1.35, help="Approximate character height for rigging")
    rig_parser.add_argument("--download-glb", action="store_true", help="Also download rigged_character_glb_url when available")
    rig_parser.add_argument("--download-basic-animations", action="store_true", help="Also download Meshy basic walking/running animations")
    rig_parser.add_argument("--poll-interval", type=int, default=DEFAULT_POLL_INTERVAL_SECONDS)
    rig_parser.add_argument("--max-wait", type=int, default=1800, help="Max wait seconds for Meshy rigging task")
    rig_parser.add_argument("--api-key-file", default=".secrets/meshy_api_key", help="Optional local file containing Meshy API key")
    rig_parser.add_argument("--skip-existing", action="store_true", help="Skip when output model/<name>_rigged.fbx already exists")
    rig_parser.add_argument("--rate-limit-retries", type=int, default=DEFAULT_RATE_LIMIT_RETRIES, help="Retries for Meshy 429 rate/queue limits")
    rig_parser.add_argument("--rate-limit-backoff", type=int, default=DEFAULT_RATE_LIMIT_BACKOFF_SECONDS, help="Base seconds for exponential 429 backoff")
    rig_parser.set_defaults(func=run_rig_character)
